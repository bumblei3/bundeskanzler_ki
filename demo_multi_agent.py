...
from core.multi_agent_system import MultiAgentSystem, QueryComplexity, AgentType
...
class MultiAgentDemo:
    ...
    def display_system_info(self):
        ...
        print(f"   Classification Domains: {', '.join(status['system_info']['classification_domains'])}")
        print()
    ...
    async def run_comprehensive_tests(self):
        ...
        for i, test_case in enumerate(test_cases, 1):
            ...
            try:
                ...
                primary_agent = response.contributing_agents[0].value if response.contributing_agents else "none"
                
                print(f"   ✅ Response received in {processing_time:.3f}s")
                print(f"   🤖 Contributing Agents: {agent_count} ({[a.value for a in response.contributing_agents]})")
    ...
    def display_test_summary(self, results: List[Dict], total_time: float):
        ...
        if successful_tests:
            avg_response_time = sum(r['processing_time'] for r in successful_tests) / len(successful_tests)
    ...
    async def interactive_demo(self):
        ...
        while True:
            try:
                ...
                print(f"\n{'='*60}")
                print(f"📊 Metadata:")
                print(f"   🤖 Contributing Agents: {[a.value for a in response.contributing_agents]}")
                print(f"   📊 Confidence: {response.confidence:.1%}")
