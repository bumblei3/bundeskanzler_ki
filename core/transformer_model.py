...
# Try to import tf_config, use fallback if not available
try:
    import tf_config
    TF_CONFIG_AVAILABLE = True
except ImportError:
    TF_CONFIG_AVAILABLE = False
    print("⚠️ tf_config nicht verfügbar - verwende Fallback")
...
class ModelBuilder:
    ...
    def __init__(self, maxlen, vocab_size, output_size):
        ...
        # Try to get config from tf_config, use fallback if not available
        if TF_CONFIG_AVAILABLE:
...
def create_transformer_model(maxlen, vocab_size, output_size):
    ...
    outputs = layers.Dense(
        ...
        activation="softmax",
        kernel_regularizer=regularizers.l1_l2(
            l1=builder.config["l1_reg"], l2=builder.config["l2_reg"]
        ),
