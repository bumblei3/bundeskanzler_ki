#!/usr/bin/env python3
"""
🚀 GPU Multi-Agent System für RTX 2070 - PRODUCTION READY
=========================================================
...
class SimpleResponseSynthesizer:
    ...
    def synthesize_responses(
        ...
        if len(responses) == 1:
            ...
            return response.get('response', 'Keine Antwort verfügbar.')
        ...
        sorted_responses = sorted(
            responses.items(),
            key=lambda x: classifications.get(x[0], 0.0),
            reverse=True
        )
        ...
        for agent_name, response in sorted_responses:
            if response and 'response' in response:
                ...
                if agent_response and len(agent_response.strip()) > 0:
                    confidence = classifications.get(agent_name, 0.0)
                    ...
                    if agent_name == 'politik':
                        ...
                        header = f"🤖 **{agent_name.title()} Perspektive** (Relevanz: {confidence:.1%}):"
            ...
            for agent_name, config in self.agent_configs.items():
                ...
                if not self.corpus_path:
                    ...
                    try:
                        ...
                        logger.warning(f"⚠️ Embedding Generation für {agent_name} fehlgeschlagen: {e}")
    ...
    async def process_query_parallel(
        ...
        try:
            ...
            active_agents = []
                ...
                # Extract domain scores from QueryClassification
                domain_scores = {}
                if classifications.primary_domain in self.agents:
                    domain_scores[classifications.primary_domain] = classifications.confidence
            
                # Add secondary domains
                for domain in classifications.secondary_domains:
                    if domain in self.agents:
                        domain_scores[domain] = classifications.confidence * 0.7  # Lower confidence for secondary
            ...
            if not active_agents:
                # Fallback: verwende besten Agent
                    best_domain = classifications.primary_domain
