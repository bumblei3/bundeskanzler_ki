...
def benchmark_gpu_performance(model, test_data, batch_size=32):
    ...
    test_sequences = test_data[: min(100, len(test_data))]  # Max 100 Samples für Benchmark

    # CPU-Zeit messen
    start_time = time.time()
    with tf.device("/CPU:0"):
        cpu_predictions = model.predict(test_sequences, batch_size=batch_size, verbose=0)
    cpu_time = time.time() - start_time

    # GPU-Zeit messen
    start_time = time.time()
    with tf.device("/GPU:0"):
        gpu_predictions = model.predict(test_sequences, batch_size=batch_size, verbose=0)
...
def load_or_train_model(tokenizer, X, Y, maxlen, args):
    ...
    if gpu_available and len(X) > 0:
        benchmark_gpu_performance(model, X[: min(100, len(X))], batch_size=args.batch_size)
