...
class MultimodalResponseGenerator:
    ...
    def generate_visual_response(self, topic: str, data_type: str = "trends",
                                time_range: str = "5_years") -> Dict[str, Any]:
        """
    ...
    def _generate_trend_chart(self, topic: str, time_range: str) -> Dict[str, Any]:
        ...
        # Plot
        ax.plot(dates, values, 'o-', color=self.colors.get(topic, '#2E86AB'),
                linewidth=3, markersize=8, alpha=0.8, label=f'{topic.title()} Entwicklung')

        # Trendlinie
        ax.plot(dates, p(range(len(values))), '--',
                color=self.colors.get(topic, '#2E86AB'), linewidth=2,
    ...
    def _generate_comparison_chart(self, topic: str) -> Dict[str, Any]:
        ...
        bars = ax.bar(categories, values, color=self.colors.get(topic, '#2E86AB'),
                     alpha=0.8, width=0.6)
        ...
        ax.set_title(f'Vergleich: {topic.title()} - Deutschland vs. International', fontsize=18, pad=20)
    ...
    def _generate_distribution_chart(self, topic: str) -> Dict[str, Any]:
        ...
        # Kreisdiagramm
        ax1.pie(values, labels=regions, autopct='%1.1f%%',
        ...
        for bar, value in zip(bars, values):
            ax2.text(value + 1, bar.get_y() + bar.get_height()/2,
                    f'{value:.1f}%', va='center', fontsize=12)
    ...
    def _generate_timeline_chart(self, topic: str) -> Dict[str, Any]:
        ...
        for i, event in enumerate(events):
            ...
            # Punkt für Ereignis
            ax.scatter(date, y_pos, s=100, color=self.colors.get(topic, '#2E86AB'),
                      edgecolor='white', linewidth=2, zorder=5)

            # Linie zum Punkt
            ax.plot([date, date], [0, y_pos], color=self.colors.get(topic, '#2E86AB'),
                   linewidth=2, alpha=0.7)

            # Text für Ereignis
            ha = 'left' if y_pos > 0 else 'right'
            va = 'bottom' if y_pos > 0 else 'top'
            ax.text(date, y_pos + (0.1 if y_pos > 0 else -0.1),
                   event['title'], ha=ha, va=va, fontsize=11,
    ...
    def _generate_error_visualization(self) -> Dict[str, Any]:
        ...
        ax.text(0.5, 0.5, 'Visualisierung\nnicht verfügbar',
    ...
    def _analyze_comparison_data(self, values: np.ndarray, categories: List[str], topic: str) -> str:
        """
