...
class AdvancedRAGSystem:
    ...
    def _build_semantic_index(self):
        ...
        try:
            ...
            self.logger.info(f"✅ Semantischer Index erstellt: {len(texts)} Embeddings, Dimension: {dimension}")
    ...
    def _tokenize_text(self, text: str) -> List[str]:
        ...
        german_stopwords = {
            ...
            'haben', 'hat', 'hatte', 'hatten', 'sein', 'seine', 'ihrer', 'ihren',
            'mit', 'von', 'zu', 'auf', 'für', 'durch', 'über', 'unter', 'zwischen',
            'nicht', 'keine', 'kein', 'nur', 'auch', 'noch', 'schon', 'wieder'
        }
    ...
    def retrieve_relevant_documents(
        ...
        try:
            ...
            self.logger.debug(f"Query '{query}' → {len(results)} Ergebnisse in {response_time:.3f}s")
    ...
    def _hybrid_search(self, query: str, top_k: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            semantic_results = self._semantic_search(query, top_k * 2)  # Mehr Ergebnisse für bessere Fusion
    ...
    def _rerank_results(self, original_query: str, results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
    ...
    def benchmark_search(self, test_queries: List[str], iterations: int = 3) -> Dict[str, Any]:
        ...
        self.logger.info(f"🔬 Starte Benchmark mit {len(test_queries)} Queries, {iterations} Iterationen")
        ...
        for method, data in results.items():
            if data["times"]:
                summary[method] = {
                    ...
                    "avg_results": sum(data["results"]) / len(data["results"]) if data["results"] else 0,
