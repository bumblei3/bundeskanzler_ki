...
class SimpleResponseSynthesizer:
    ...
    def synthesize_responses(
        ...
        sorted_responses = sorted(
            responses.items(),
            key=lambda x: domain_scores.get(x[0], 0.0),
            reverse=True
        )
    ...
    async def process_query_parallel(
        ...
        try:
            ...
            if classification_result.primary_domain in self.agents:
                domain_scores[classification_result.primary_domain] = classification_result.confidence
            
            # Add secondary domains
            for domain in classification_result.secondary_domains:
                if domain in self.agents:
                    domain_scores[domain] = classification_result.confidence * 0.7  # Lower confidence for secondary
