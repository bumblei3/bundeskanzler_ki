...
class BaseAgent(ABC):
    ...
    def __init__(
        self, agent_type: AgentType, capability: AgentCapability, rag_system: AdvancedRAGSystem
    ):
    ...
    def calculate_query_relevance(self, query: str) -> float:
        ...
        keyword_matches = sum(
            1 for keyword in self.capability.keywords if keyword.lower() in query_lower
        )
        ...
        expertise_matches = sum(
            1 for area in self.capability.expertise_areas if area.lower() in query_lower
        )
...
class WirtschaftAgent(BaseAgent):
    ...
    def _filter_wirtschaft_documents(self, documents: List[Dict]) -> List[Dict]:
        ...
        filtered_docs.sort(
            key=lambda x: (x.get("wirtschaft_relevance", 0), x["score"]), reverse=True
        )
    ...
    def _generate_wirtschaft_response(self, query: str, documents: List[Dict]) -> str:
        ...
        if not documents:
            return (
                "Ich konnte keine relevanten wirtschaftlichen Informationen zu Ihrer Frage finden."
            )
    ...
    def _calculate_response_confidence(self, documents: List[Dict]) -> float:
        ...
        avg_score = sum(doc["score"] for doc in documents) / len(documents)
        avg_relevance = sum(doc.get("wirtschaft_relevance", 0) for doc in documents) / len(
            documents
        )
...
class CoordinatorAgent:
    ...
    def _synthesize_responses(
        self, query: str, classification: QueryClassification, responses: List[AgentResponse]
    ) -> MultiAgentResponse:
        ...
        if not responses:
            ...
            try:
                ...
                fallback_content = (
                    documents[0]["text"] if documents else "Keine Informationen gefunden."
                )
        ...
        synthesized_content += (
            f"📊 **Zusammenfassung:** Analyse von {len(responses)} Fachbereichen "
        )
