...
class ProductionGPUHealthMonitor:
    ...
    def _collect_health_metrics(self) -> HealthMetrics:
        ...
        if gpu_available and gpu_stats:
            if (gpu_stats.temperature_c > self.thresholds['gpu_temperature_critical'] or
                gpu_stats.memory_utilization > self.thresholds['gpu_memory_critical']):
                system_healthy = False
        
        if (cpu_usage > self.thresholds['cpu_usage_warning'] or 
            ram_usage > self.thresholds['ram_usage_warning']):
            system_healthy = False
...
class ProductionGPUManager:
    ...
    async def process_query_production(
        ...
        try:
            ...
            use_gpu = (
                health.gpu_available and 
                health.system_healthy and
                health.gpu_temperature < self.health_monitor.thresholds['gpu_temperature_warning']
            )
            ...
            try:
                ...
                logger.info(f"✅ Query {query_id} erfolgreich verarbeitet in {processing_time:.1f}ms")
                return result
    ...
    def _optimize_batch_size(self):
        ...
        try:
            ...
            if optimal_batch != self.performance_stats['current_batch_size']:
                logger.info(f"🔧 Batch Size optimiert: {self.performance_stats['current_batch_size']} → {optimal_batch}")
    ...
    def get_production_summary(self) -> Dict[str, Any]:
        ...
        gpu_usage_ratio = (
            self.performance_stats['gpu_queries'] / total_queries * 100 
        ...
        return {
            ...
            'gpu_info': {
                ...
                'memory_total_gb': 8.0,
                'cuda_streams_active': len(self.multi_agent.cuda_streams) if self.multi_agent else 0
            }
        }
