...
from core.multilingual_manager import get_multilingual_manager, Language
...
class VerbesserteBundeskanzlerKI:
    ...
    def antwort(self, frage: str, user_id: str = "anonymous", feedback: Optional[Dict[str, Any]] = None) -> dict:
        """
        ...
        try:
            ...
            if detected_lang.value != "de":
                translation_result = self.multilingual_manager.translate_text(frage, Language.DE, detected_lang)
            ...
            personalized_recommendations = self.profile_manager.get_personalized_recommendations(user_id)

            # 🛡️ SICHERHEITSPRÜFUNGEN

            # 1. Input-Validation
            is_valid, validation_reason, validation_metadata = self.security_manager.validate_input(frage, user_id)
            ...
            if thema_docs:
                ...
                content_allowed, filter_reason, filter_metadata = self.security_manager.filter_content(
                    antwort_text, beste_antwort
                )
                ...
                if len(thema_docs) > 1 and konfidenz > 50:  # Threshold für Prozentwerte anpassen
                    zusatz_info = [
                        doc["text"] for doc in thema_docs[1:3] if doc["score"] * 100 > 30  # Threshold für Prozentwerte anpassen
                    ]
                    if zusatz_info:
            ...
            if detected_lang.value != "de":
                antwort_translation = self.multilingual_manager.translate_text(antwort_text, detected_lang, Language.DE)
                antwort_text = antwort_translation.translated_text
                print(f"🌍 Antwort übersetzt: {antwort_translation.original_text[:50]}... -> {antwort_text[:50]}...")
            ...
            return {
                ...
                "erklaerung": beste_antwort.get("explanation", "Keine detaillierte Erklärung verfügbar."),
                ...
                "personalization_level": personalized_recommendations.get("personalization_level", "basic"),
    ...
    def _personalisiere_antwort(self, antwort_text: str, user_profile, recommendations: dict, konfidenz: float) -> str:
    ...
    while True:
        try:
            ...
            if security_status == 'approved':
                ...
                print(f"📊 Ethik-Scores: Transparenz={transparency:.1f}, Fairness={fairness:.1f}, Accountability={accountability:.1f}")
                ...
                print(f"🛡️ Sicherheit: ❌ Blockiert - {result.get('block_reason', 'Unbekannter Grund')}")
            elif security_status == 'filtered':
                print(f"🛡️ Sicherheit: ⚠️ Gefiltert - {result.get('filter_reason', 'Unbekannter Grund')}")
            ...
            if personalization_level != 'basic':
                print(f"🧠 Personalisierung: {personalization_level.title()} (Interaktionen: {interaction_count})")
                if suggested_topics:
                ...
                try:
                    ...
                    if feedback_input and feedback_input.isdigit():
                        ...
                        if 1 <= feedback_score <= 5:
                            ...
                            print(f"✅ Feedback gespeichert! Danke für Ihre Bewertung ({feedback_score}/5)")
                        else:
...
def generiere_multimodale_antwort(ki_system: VerbesserteBundeskanzlerKI, frage: str, user_id: str = "anonymous",
                                 visual_type: str = "auto") -> dict:
    """
