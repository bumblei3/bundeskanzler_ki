...
from .rtx2070_llm_manager import get_rtx2070_llm_manager, generate_llm_response
...
class RTX2070BundeskanzlerKI:
        ...
        try:
            ...
            if self.config.enable_rag_optimization:
                ...
                if not self.rag_system.build_index():
                    logger.warning("⚠️ RTX 2070 RAG konnte nicht initialisiert werden, verwende Fallback")
                    self.rag_system = None
                else:
        ...
        try:
            ...
            if self.multilingual_manager:
                try:
                    ...
                    logger.info(f"🌍 Sprache erkannt: {query_language} (Vertrauen: {confidence:.2f})")
            ...
            if self.config.enable_llm_integration and query_complexity in ["complex", "expert"]:
                enhanced_response = self._enhance_with_llm(translated_query, agent_response, rag_context)
            else:
            ...
            return {
                ...
                'original_response': enhanced_response if final_response != enhanced_response else None,
    ...
    def _analyze_query_complexity(self, query: str) -> str:
        ...
        if query_length < 5:
            ...
            return "moderate"
        elif any(keyword in query.lower() for keyword in ["politik", "gesetz", "bundestag", "regierung"]):
            return "expert"
        else:
            return "complex"
        ...
        try:
            if self.rag_system and self.config.enable_rag_optimization:
                ...
                rag_result = self.rag_system.rag_query(
                    query,
                    top_k=3 if complexity == "simple" else 5
                )
    ...
    def _route_to_agents(self, query: str, context: str, complexity: str) -> str:
        ...
        try:
            ...
            response = loop.run_until_complete(
                self.multi_agent_system.query(query, context_dict)
            )
            loop.close()
    ...
    def _select_agent(self, query: str) -> AgentType:
        ...
        if any(word in query_lower for word in ["bundestag", "gesetz", "politik", "wahl", "partei"]):
            return AgentType.POLITIK
        elif any(word in query_lower for word in ["wirtschaft", "finanzen", "arbeitsmarkt", "inflation", "eu"]):
            return AgentType.WIRTSCHAFT
        elif any(word in query_lower for word in ["klima", "energie", "umwelt", "co2", "erneuerbare"]):
