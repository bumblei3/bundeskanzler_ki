...
Bitte gib eine kohärente Antwort, die den bisherigen Kontext berücksichtigt:""",
    ...
    def add_conversation_context(self, user_id: str, message: Dict):
        ...
        if len(self.conversation_context[user_id]) > 10:
            self.conversation_context[user_id] = self.conversation_context[user_id][
                -10:
            ]
    ...
    def fact_check_response(
        self, response: str, topic: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        for fact_key, fact_value in facts.items():
            if (
                fact_key.lower() in response_lower
                or fact_value.lower() in response_lower
            ):
        ...
        if verified_count > 0:
            ...
            result["suggestions"].append(
                "Keine verifizierten Fakten in der Antwort gefunden"
            )
    ...
    def calculate_response_quality_score(
        ...
        if len(response.split()) > 20:
            ...
            quality_metrics["improvement_suggestions"].append(
                "Antwort mit mehr Details ausfüllen"
            )
        ...
        if response.count(".") > 2 and len(response) > 50:
            ...
            quality_metrics["improvement_suggestions"].append(
                "Antwort strukturierter gestalten"
            )
        ...
        helpful_score = sum(
            1 for indicator in helpful_indicators if indicator in response.lower()
        )
        ...
        overall_score = sum(
            quality_metrics["criteria"][k] * v for k, v in weights.items()
        )
