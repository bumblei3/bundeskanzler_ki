...
class CodeQualityManager:
    ...
    def _run_mypy(self, paths: List[str]) -> Dict:
        ...
        try:
            cmd = [sys.executable, "-m", "mypy", "--json-report", str(self.report_dir)] + paths
            result = subprocess.run(cmd, cwd=self.project_root, capture_output=True, text=True)
    ...
    def _run_bandit(self, paths: List[str]) -> Dict:
        ...
        try:
            cmd = [sys.executable, "-m", "bandit", "-r", "-f", "json", "-o", 
                   str(self.report_dir / "bandit_report.json")] + paths
            result = subprocess.run(cmd, cwd=self.project_root, capture_output=True, text=True)
            ...
            if report_file.exists():
                ...
                print(f"   🔒 Bandit Security Issues: {len(report.get('results', []))} (High: {high_issues}, Medium: {medium_issues})")
    ...
    def _generate_summary(self, format_results: Dict, lint_results: Dict, security_results: Dict) -> Dict:
