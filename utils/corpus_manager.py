...
class CorpusManager:
    ...
    def load_corpus(self) -> None:
        ...
        if os.path.exists(self.corpus_file):
                    ...
                    if "entries" in data:
                        ...
                        for entry in data["entries"]:
                            if isinstance(entry, dict) and "text" in entry:
                                ...
                                logging.warning(
                                    f"Ungültiger Eintrag im Korpus: {entry}"
                                )
                    print(
                        f"Geladener Korpus enthält {sum(len(v) for v in self.corpus.values())} Einträge"
                    )
                    logging.info(
                        f"Korpus geladen aus {self.corpus_file} mit {sum(len(v) for v in self.corpus.values())} Einträgen"
                    )
            ...
            logging.warning(
                f"{self.corpus_file} nicht gefunden, initialisiere Standard-Korpus"
            )
            self._initialize_default_corpus()
    ...
    def _deduplicate_corpus(self) -> None:
        ...
        if removed_duplicates > 0:
            print(
                f"Duplikate entfernt: {removed_duplicates} Einträge (von {original_count} auf {deduplicated_count})"
            )
    ...
    def add_sentence(self, sentence: str, category: str, language: str) -> None:
        ...
        self.corpus[category].append({"text": sentence, "language": language})
        ...
        for entry in additional_data:
            ...
            if entry["text"] not in existing_texts:
                self.corpus[category].append(
                    {"text": entry["text"], "language": entry["language"]}
                )
    ...
    def generate_additional_training_data(
        self, num_entries_per_category: int = 3
    ) -> None:
        """Generiert zusätzliche Trainingsdaten basierend auf vorhandenen Mustern."""
        print(
            f"🤖 Generiere zusätzliche Trainingsdaten ({num_entries_per_category} pro Kategorie)..."
        )
        ...
        for category, patterns in generation_patterns.items():
            ...
            for i in range(num_entries_per_category):
                ...
                if text not in existing_texts:
                    self.corpus[category].append({"text": text, "language": "de"})
                    generated_count += 1
