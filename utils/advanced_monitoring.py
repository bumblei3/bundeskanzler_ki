...
class AdvancedMonitoringSystem:
    ...
    def log_user_feedback(
        ...
        if rating <= 2:
            logger.warning(
                f"👎 Niedrige Bewertung ({rating}/5) für Frage: {question[:50]}..."
            )
        ...
        if session_id in self.active_sessions:
            ...
            if session["interactions"] > 0:
                session["avg_response_time"] = (
                    session["total_response_time"] / session["interactions"]
                )

            logger.info(
                f"📊 Session {session_id} beendet: {session['interactions']} Interaktionen, "
    ...
    def update_session_metrics(
        self, session_id: str, response_time: float, quality_score: float
    ):
        ...
        if session_id in self.active_sessions:
            ...
            session["avg_quality_score"] = (
                (session["avg_quality_score"] * (session["interactions"] - 1))
                + quality_score
    ...
    def get_system_health_report(self) -> Dict[str, Any]:
        ...
        return {
            "status": "healthy",
            "timestamp": datetime.now().isoformat(),
            "cpu_avg_percent": statistics.mean(
                m["cpu_percent"] for m in recent_metrics
            ),
            "memory_avg_percent": statistics.mean(
    ...
    def get_performance_report(self, hours: int = 1) -> Dict[str, Any]:
        ...
        recent_resp = [
            m
            for m in self.response_metrics
            if datetime.fromisoformat(m["timestamp"]) > cutoff_time
        ]
        ...
        report = {
            ...
            "avg_response_time_ms": (
                statistics.mean(m["response_time_ms"] for m in recent_resp)
                if recent_resp
                else 0
            ),
            "avg_quality_score": (
                statistics.mean(m["quality_score"] for m in recent_resp)
                if recent_resp
                else 0
            ),
        ...
        if recent_perf:
            report["success_rate"] = (
                report["successful_operations"] / report["total_operations"]
            )
        else:
            ...
            self.hourly_stats[current_hour.isoformat()].append(
                {
                    "avg_duration_ms": statistics.mean(
                        m["duration_ms"] for m in recent_perf
                    ),
                    "success_rate": sum(1 for m in recent_perf if m["success"])
                    / len(recent_perf),
                }
            )
            ...
            self.daily_stats[current_day.isoformat()].append(
                {
                    "avg_quality": statistics.mean(
                        m["quality_score"] for m in recent_resp
                    ),
                    "avg_response_time": statistics.mean(
                        m["response_time_ms"] for m in recent_resp
                    ),
                    "total_responses": len(recent_resp),
                }
            )
    ...
    def _cleanup_old_data(self):
        ...
        to_remove = [
            k
            for k in self.hourly_stats.keys()
        ...
        to_remove = [
            k for k in self.daily_stats.keys() if datetime.fromisoformat(k) < cutoff_day
        ]
        for k in to_remove:
            ...
            if os.path.exists(history_file):
                ...
                if "performance_metrics" in data:
                    self.performance_metrics.extend(
                        data["performance_metrics"][-self.max_history :]
                    )
                if "response_metrics" in data:
                    self.response_metrics.extend(
                        data["response_metrics"][-self.max_history :]
                    )
                if "user_feedback" in data:
                    self.user_feedback.extend(
                        data["user_feedback"][-self.max_history :]
                    )

                logger.info(
                    f"📚 Historische Monitoring-Daten geladen: {len(self.performance_metrics)} Performance, "
    ...
    def export_report(
        self, report_type: str = "full", file_path: Optional[str] = None
    ) -> str:
        """
