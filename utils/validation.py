...
def validate_model(
    ...
    with open(test_file, "r", encoding="utf-8") as ftest:
        ...
        for idx, line in enumerate(lines):
            ...
            if (idx + 1) % 10 == 0 or (idx + 1) == len(lines):
                ...
                eta = (
                    (elapsed / (idx + 1)) * (len(lines) - (idx + 1))
                    if idx + 1 < len(lines)
                    else 0
                )
                print(
                    f"[{bar}] {percent*100:.1f}% | ETA: {eta:.1f}s",
                    end="\r" if (idx + 1) != len(lines) else "\n",
                )
    if total > 0:
        print(f"Test-Genauigkeit (Top-1): {correct}/{total} = {correct/total*100:.1f}%")
        if top_n > 1:
            print(
                f"Test-Genauigkeit (Top-{top_n}): {correct_topn}/{total} = {correct_topn/total*100:.1f}%"
            )
        if threshold is not None:
            print(
                f"Schwellenwert: {threshold}, gezählt nur wenn Wahrscheinlichkeit >= Schwelle"
            )
        # Durchschnittliche Wahrscheinlichkeiten
        if probs_correct:
            print(
                f"Durchschnittliche Wahrscheinlichkeit (korrekt): {np.mean(probs_correct):.4f}"
            )
        if probs_incorrect:
            print(
                f"Durchschnittliche Wahrscheinlichkeit (inkorrekt): {np.mean(probs_incorrect):.4f}"
            )
        # Warnung bei seltenen Klassen
        few_samples = [cls for cls, cnt in class_counts.items() if cnt < 3]
        if few_samples:
            print(
                f"Warnung: Folgende Klassen haben weniger als 3 Testdaten: {few_samples}"
            )
    else:
    ...
    if log_file:
        with open(log_file, "a", encoding="utf-8") as flog:
            flog.write(
                f"Validierung {test_file}: Top-1={correct}, Top-{top_n}={correct_topn}, Total={total}, Fehler={errors}, Schwelle={threshold}\n"
            )
    ...
    for r in error_examples:
        print(
            f"Input: {r['input']} | Erwartet: {r['expected']} | Vorhergesagt: {r['predicted']} | Prob_richtig: {r['prob_pred']:.4f}"
        )
