...
class EnhancedSecuritySystem:
    ...
    def _load_forbidden_words(self) -> Set[str]:
        ...
        return {
            # Offensive Sprache
            "verdammt", "scheiße", "arsch", "wichser", "fotze", "schwanz",
            # Diskriminierung
            "nazi", "hitler", "juden", "neger", "schwuchtel", "kanake",
            # Illegale Aktivitäten
            "drogen", "kokain", "heroin", "meth", "ecstasy", "waffe", "bombe",
            "mord", "töten", "erschießen", "vergewaltigen",
            # Politisch sensible Themen
            "umsturz", "revolution", "putsch", "terror", "anschlag"
        }
    ...
    def _load_sensitive_topics(self) -> Dict[str, List[str]]:
        ...
        return {
            ...
            "geheimdienste": ["bundesnachrichtendienst", "verfassungsschutz", "nsa", "cia"],
            "militär": ["bundeswehr", "atomwaffe", "nuklear", "krieg"],
            "gesundheit": ["corona", "pandemie", "impfung", "nebenwirkung"],
            "politik_extrem": ["rechtsradikal", "linksextrem", "extremismus", "radikalisierung"]
        }
    ...
    def validate_input(self, input_text: str, user_id: str = "anonymous",
        ...
        if self.enable_rate_limiting:
            ...
            if not rate_limit_result["allowed"]:
                ...
                result["recommendations"].append("Bitte warten Sie einen Moment vor der nächsten Anfrage")
    ...
    def check_rate_limit(self, user_id: str, ip_address: str) -> Dict[str, Any]:
        ...
        while self.request_history[identifier] and current_time - self.request_history[identifier][0] > 3600:
            self.request_history[identifier].popleft()

        # Zähle Requests in letzten 60 Sekunden
        minute_ago = current_time - 60
        recent_requests = sum(1 for t in self.request_history[identifier] if t > minute_ago)

        # Zähle Requests in letzten 3600 Sekunden
        hour_ago = current_time - 3600
        hourly_requests = sum(1 for t in self.request_history[identifier] if t > hour_ago)

        allowed = (recent_requests < self.max_requests_per_minute and
                  hourly_requests < self.max_requests_per_hour)

        if not allowed:
            logger.warning(f"Rate limit exceeded for {identifier}: {recent_requests}/min, {hourly_requests}/hour")
    ...
    def filter_content(self, input_text: str) -> Dict[str, Any]:
        ...
        if found_forbidden:
            ...
            result["warnings"].append(f"Verbotene Wörter gefunden: {', '.join(found_forbidden[:3])}")
    ...
    def detect_threats(self, input_text: str, user_id: str) -> Dict[str, Any]:
        ...
        for pattern_info in self.threat_patterns:
            ...
            if pattern.search(input_text):
                ...
                result["risk_score"] += 0.8 if pattern_info["severity"] == "high" else 0.4

                if pattern_info["action"] == "block":
                    result["blocked"] = True

                logger.warning(f"Threat detected: {pattern_info['name']} from user {user_id}")
        ...
        if self.anomaly_scores[user_id] > 3.0:
            ...
            logger.warning(f"High anomaly score for user {user_id}: {self.anomaly_scores[user_id]}")
    ...
    def get_security_report(self) -> Dict[str, Any]:
        ...
        blocked_requests = sum(1 for e in recent_events if not e.get("validation_result", True))
        high_risk_requests = sum(1 for e in recent_events if e.get("risk_score", 0) > 0.5)
        ...
        return {
            ...
            "rate_limited_users": len([h for h in self.request_history.values() if len(h) > 50])
        }
    ...
    def export_security_report(self, file_path: str):
        ...
        report = {
            ...
            "high_anomaly_users": {k: v for k, v in self.anomaly_scores.items() if v > 1.0}
        }
