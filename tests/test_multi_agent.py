    ...
    def test_complex_multi_domain_classification(self):
        ...
        for query in complex_queries:
            ...
            self.assertGreaterEqual(
                len(classification.secondary_domains),
                1,
...
class TestAgentCapabilities(unittest.TestCase):
    ...
    def test_politik_agent_relevance_calculation(self):
        ...
        self.assertGreater(
            high_relevance, low_relevance, "Politik agent should rate politik queries higher"
        )
        self.assertGreater(high_relevance, 0.5, "High relevance politik query should score > 0.5")
    ...
    def test_wirtschaft_agent_relevance_calculation(self):
        ...
        self.assertGreater(
            high_relevance, low_relevance, "Wirtschaft agent should rate wirtschaft queries higher"
        )
    ...
    def test_klima_agent_relevance_calculation(self):
        ...
        self.assertGreater(
            high_relevance, low_relevance, "Klima agent should rate klima queries higher"
        )
...
class TestCoordinatorAgent(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_single_agent_response(self):
        ...
        with patch.object(
            self.coordinator.agents[AgentType.POLITIK], "process_query", return_value=mock_response
        ):
    ...
    async def test_multi_agent_response_synthesis(self):
        ...
        with patch.object(
            self.coordinator.agents[AgentType.POLITIK],
            "process_query",
            return_value=politik_response,
        ), patch.object(
            self.coordinator.agents[AgentType.WIRTSCHAFT],
            "process_query",
            return_value=wirtschaft_response,
        ), patch.object(
            self.coordinator.agents[AgentType.KLIMA], "process_query", return_value=klima_response
        ):
...
class TestMultiAgentSystem(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_query_interface(self):
        ...
        with patch.object(self.system.coordinator, "process_query", return_value=mock_response):
...
class TestPerformance(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_response_time_performance(self):
        ...
        with patch.object(self.system.coordinator, "process_query", return_value=mock_response):
            ...
            self.assertLess(
                avg_time_per_query,
                1.0,  # Should be < 1 second per query
    ...
    async def test_concurrent_query_handling(self):
        ...
        queries = ["Politik Test 1", "Wirtschaft Test 2", "Klima Test 3"]
