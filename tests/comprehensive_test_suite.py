...
class TestPerformanceKI:
    ...
    def test_performance_stats(self, temp_project_dir):
        ...
        with patch('core.performance_ki.RAGSystem'), \
             patch('core.performance_ki.project_root', str(temp_project_dir)):
...
class TestRAGSystem:
    ...
    def test_rag_initialization(self, mock_faiss, mock_sentence_transformer, temp_rag_dir):
        ...
        assert rag.corpus is not None
        assert len(rag.corpus) == 3
        mock_sentence_transformer.assert_called_once()
    
    @patch('sentence_transformers.SentenceTransformer')
    @patch('faiss.IndexFlatIP')
    def test_document_retrieval(self, mock_faiss, mock_sentence_transformer, temp_rag_dir):
...
class TestSystemIntegration:
    ...
    @patch('faiss.IndexFlatIP') 
    def test_full_system_integration(self, mock_faiss, mock_transformer, integration_env):
