...
def memory_system(temp_memory_path):
    ...
    return HierarchicalMemory(base_path=temp_memory_path, levels=3)
...
def test_search_by_content(memory_system):
    ...
    memory_system.add_memory(
        "Economic growth strategies", np.random.rand(512).astype(np.float32), 0.7
    )
...
def test_memory_stats(memory_system):
    ...
    for i in range(3):
        memory_system.add_memory(
            f"Memory {i}", np.random.rand(512).astype(np.float32), 0.5 + i * 0.1
        )
...
def test_memory_persistence(temp_memory_path):
    ...
    memory = HierarchicalMemory(base_path=temp_memory_path, levels=3)

    # Füge Erinnerung hinzu
    embedding = np.random.rand(512).astype(np.float32)
    item = MemoryItem(content="Persistent memory", embedding=embedding, importance=0.8)
...
def test_enhanced_context_processor(temp_memory_path):
    ...
    # Füge Kontext hinzu
    embedding = np.random.rand(512).astype(np.float32)
    metadata = {"source": "government", "tags": ["policy", "climate"], "urgency": 0.7}
