...
# Import der API
from bundeskanzler_api import app, APIConfig
...
def test_login_valid_credentials(client):
    ...
    response = client.post(
        "/auth/token",
        data={"username": "bundeskanzler", "password": "ki2025"}
    )
    assert response.status_code == 200
...
def test_search_memory(client, auth_headers):
    ...
    assert response.status_code in [200, 500]  # 500 might be expected if memory system has issues
...
def test_response_time(client, auth_headers):
    ...
    response = client.post(
        "/chat",
        json={"message": "Schnelle Antwort bitte"},
        headers=auth_headers
    )
