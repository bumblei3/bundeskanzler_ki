...
class GPUManager:
    ...
    def setup_gpus(self) -> None:
        ...
        if not self.gpus:
            ...
            self.distribution_strategy = tf.distribute.OneDeviceStrategy(device="/cpu:0")
            return
        ...
        try:
            ...
            logging.info(f"GPU-Konfiguration erfolgreich. Verfügbare GPUs: {len(self.gpus)}")
            for gpu in self.gpus:
...
def get_optimal_training_config() -> Dict[str, Any]:
    ...
    try:
        ...
        if gpus:
            ...
            memory_limit = tf.config.get_logical_device_configuration(
                gpus[0]
            )[0].memory_limit if tf.config.get_logical_device_configuration(gpus[0]) else None
