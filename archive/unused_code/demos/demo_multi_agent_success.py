...
async def quick_demo():
    ...
    try:
        ...
        for agent_name, info in capabilities.items():
            ...
            print(
                f"{emoji} {agent_name.upper()}: {len(info['expertise_areas'])} Expertise Areas, {info['keyword_count']} Keywords"
            )
        ...
        for domain, query in test_queries:
            ...
            try:
                ...
                preview = (
                    response.primary_response[:100] + "..."
                    if len(response.primary_response) > 100
                    else response.primary_response
                )
        ...
        print(
            f"   Success Rate: {coordinator_stats['successful_responses']}/{coordinator_stats['queries_processed']}"
        )
        if coordinator_stats["queries_processed"] > 0:
            success_rate = (
                coordinator_stats["successful_responses"]
                / coordinator_stats["queries_processed"]
                * 100
            )
