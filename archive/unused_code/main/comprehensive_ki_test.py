...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("logs/comprehensive_ki_test.log"), logging.StreamHandler()],
)
...
class BundeskanzlerKITestSuite:
    ...
    def test_tensor_cores(self) -> Tuple[bool, str, Dict]:
        ...
        try:
            ...
            if tensor_cores > 0 and fp16_enabled:
                return (
                    True,
                    f"Tensor Cores aktiv: {tensor_cores}% Auslastung",
                    {"tensor_core_usage": tensor_cores, "fp16_enabled": fp16_enabled},
                )
            else:
                return (
                    False,
                    "Tensor Cores nicht verfügbar oder nicht aktiviert",
                    {"tensor_core_usage": tensor_cores, "fp16_enabled": fp16_enabled},
                )
    ...
    def test_rag_system_initialization(self) -> Tuple[bool, str, Dict]:
        ...
        try:
            ...
            rag_available = components.get("rtx2070_rag", False) or components.get(
                "fallback_rag", False
            )
    ...
    def test_corpus_loading(self) -> Tuple[bool, str, Dict]:
        ...
        try:
            ...
            if corpus_loaded and corpus_entries > 0:
                return (
                    True,
                    f"Corpus geladen: {corpus_entries} Einträge",
                    {
                ...
                return (
                    False,
                    "Corpus nicht geladen oder leer",
                    {"corpus_entries": corpus_entries, "corpus_loaded": corpus_loaded},
                )
    ...
    def test_embedding_generation(self) -> Tuple[bool, str, Dict]:
        ...
        try:
            ...
            if embedding_dimension > 0 and embedding_model:
                return (
                    True,
                    f"Embeddings verfügbar: {embedding_model} (Dim: {embedding_dimension})",
                    {
                ...
                return (
                    False,
                    "Embedding-System nicht verfügbar",
                    {
                        "embedding_dimension": embedding_dimension,
                        "embedding_model": embedding_model,
                    },
            ...
            if multi_agent_available:
                return (
                    ...
                    "Multi-Agent System verfügbar",
                    {
                        ...
                        "agents_initialized": system_info.get("multi_agent_info", {}).get(
        ...
        try:
            ...
            if result and result.get("response"):
                ...
                return (
                    ...
                    {"query": test_query, "result": result, "duration": duration},
                )
    ...
    def test_multiple_queries(self) -> Tuple[bool, str, Dict]:
        ...
        for query in queries:
            try:
                ...
                results.append({"query": query, "success": False, "error": str(e), "duration": 0})
        ...
        return (
            success,
            f"Multiple Queries: {successful_queries}/{len(queries)} erfolgreich (Ø {avg_duration:.2f}s)",
            {
    ...
    def test_query_consistency(self) -> Tuple[bool, str, Dict]:
        ...
        if len(non_empty_responses) >= 2:
            ...
            return (
                False,
                "Nicht genügend erfolgreiche Antworten für Konsistenz-Test",
                {"responses": responses},
            )
    ...
    def test_performance_baseline(self) -> Tuple[bool, str, Dict]:
        ...
        if valid_durations:
            ...
            return (
                False,
                "Alle Performance-Test-Läufe fehlgeschlagen",
                {"iterations": iterations, "valid_runs": 0},
            )
    ...
    def test_concurrent_queries(self) -> Tuple[bool, str, Dict]:
        ...
        return (
            success,
            f"Concurrent Queries: {successful}/{total} erfolgreich (Ø {avg_duration:.2f}s)",
            {
                "total_queries": total,
        ...
        try:
            ...
            return (
                ...
                ".1f",
                {
                    ...
                    "avg_query_duration": sum(r["duration"] for r in session_results)
                    / len(session_results),
            ...
            return (
                False,
                f"Session-Test Fehler nach {session_duration:.1f}s: {e}",
                {
                    "session_duration": session_duration,
    ...
    def generate_report(self, suite_results: List[TestSuiteResult]) -> str:
        ...
        if self.ki:
            try:
                ...
                if gpu_info:
                    ...
                    report.append(
                        f"  • Speichernutzung: {gpu_info.get('memory_used_gb', 0):.1f}GB / {gpu_info.get('memory_total_gb', 0):.1f}GB"
                    )
        ...
        if success_rate >= 90:
            ...
            report.append(
                "  ⚠️ Gute KI-Performance. Einige kleinere Probleme sollten behoben werden."
            )
        else:
    ...
    def run_all_tests(
        self, include_performance: bool = True, include_stress: bool = True
...
def main():
    ...
    results = test_suite.run_all_tests(
        include_performance=args.performance, include_stress=args.stress
    )
