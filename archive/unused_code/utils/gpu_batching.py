...
class GPUBatchProcessor:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        if self.gpu_available:
            try:
                ...
                stats["gpu_memory_total_mb"] = torch.cuda.get_device_properties(0).total_memory / (
                    1024 * 1024
                )
            except:
                pass
        ...
        if self.enable_memory_pooling and self.memory_pool is not None:
            ...
            # Effizienz berechnen
            total_pool_accesses = (
                stats["memory_pool_stats"]["hits"] + stats["memory_pool_stats"]["misses"]
            )
    ...
    async def process_batch_async(
        ...
        return await loop.run_in_executor(self.executor, self.process_batch_sync, texts, operation)

    def process_batch_sync(self, texts: List[str], operation: str = "embed") -> np.ndarray:
        """
    ...
    def _gpu_embed_batch(self, batch: List[str]) -> np.ndarray:
        ...
        try:
            ...
            if self.memory_pool and batch_size <= 64:
                pool_key = (
                    "small" if batch_size <= 16 else "medium" if batch_size <= 32 else "large"
                )
...
class AsyncBatchManager:
    ...
    async def submit_batch_task(
        ...
        async with self.task_lock:
            ...
            task = asyncio.create_task(self.gpu_processor.process_batch_async(texts, operation))
