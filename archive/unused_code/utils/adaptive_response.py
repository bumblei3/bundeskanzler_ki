...
class AdaptiveResponseManager:
    ...
    def __init__(self, complexity_levels: int = 5, min_tokens: int = 20, max_tokens: int = 500):
        self.complexity_levels = complexity_levels
        self.min_tokens = min_tokens
        self.max_tokens = max_tokens
        self.user_profiles = {}

    def get_complexity_params(self, user_id: str, context: Dict) -> Dict[str, float]:
        """
        ...
        target_complexity = self._adjust_complexity(
            base_complexity, context_complexity, profile["learning_rate"]
        )
    ...
    def update_user_profile(self, user_id: str, interaction_data: Dict) -> None:
        """
        ...
        if success < 0.3:  # Zu komplex
            ...
            profile["base_complexity"] = min(1.0, profile["base_complexity"] * 1.1)

        # Lernrate anpassen
        profile["learning_rate"] = max(
            0.1, min(1.0, profile["learning_rate"] + (success - 0.5) * 0.1)
        )
    ...
    def _adjust_complexity(self, base: float, context: float, learning_rate: float) -> float:
