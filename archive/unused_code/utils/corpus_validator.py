        ...
        for idx, sentence in enumerate(sentences):
            ...
            if is_valid:
                ...
                results["invalid_sentences"].append(
                    {"index": idx, "sentence": sentence, "problems": problems}
                )
...
def print_validation_report(results: Dict) -> None:
    ...
    logging.info(
        f"Valide Sätze: {results['valid_sentences']} ({results['valid_sentences']/results['total_sentences']*100:.1f}%)"
    )
    ...
    logging.info("\nStatistiken:")
    logging.info(
        f"  Durchschnittliche Satzlänge: {results['statistics']['avg_length']:.1f} Zeichen"
    )
    logging.info("\n  Sprachen:")
