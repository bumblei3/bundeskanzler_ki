...
class FactChecker:
    ...
    def _load_source_credibility(self) -> Dict[str, SourceCredibility]:
        ...
        try:
            with open(self.credibility_db_path, "r", encoding="utf-8") as f:
                data = json.load(f)
                return {domain: SourceCredibility(**info) for domain, info in data.items()}
    ...
    def _create_default_credibility_db(self) -> Dict[str, SourceCredibility]:
        ...
        return {domain: SourceCredibility(**info) for domain, info in default_sources.items()}
    ...
    def _generate_explanation(
        ...
        if abs(bias_score) > 0.3:
            ...
            explanation += (
                f" Die Aussage zeigt eine {bias_direction}e Tendenz (Bias-Score: {bias_score:.2f})."
            )
