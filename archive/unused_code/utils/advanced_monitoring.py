...
class AdvancedMonitoringSystem:
    ...
    def get_performance_report(self, hours: int = 1) -> Dict[str, Any]:
        ...
        recent_resp = [
            m for m in self.response_metrics if datetime.fromisoformat(m["timestamp"]) > cutoff_time
        ]
        ...
        report = {
            ...
            "avg_response_time_ms": (
                statistics.mean(m["response_time_ms"] for m in recent_resp) if recent_resp else 0
            ),
            "avg_quality_score": (
                statistics.mean(m["quality_score"] for m in recent_resp) if recent_resp else 0
            ),
    ...
    def _cleanup_old_data(self):
        ...
        to_remove = [k for k in self.hourly_stats.keys() if datetime.fromisoformat(k) < cutoff_hour]
        for k in to_remove:
            del self.hourly_stats[k]

        # Bereinige tägliche Stats
        to_remove = [k for k in self.daily_stats.keys() if datetime.fromisoformat(k) < cutoff_day]
        for k in to_remove:
